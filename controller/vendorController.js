  ...
  try {
    ...
    if (!vendorDetails) {
      ...
      if (storeName !== undefined) {
        vendorDetails.storeName = storeName
          ? String(storeName).trim()
          : null;
      }

      if (storeType !== undefined) {
        vendorDetails.storeType = storeType
          ? String(storeType).trim()
          : null;
      }

      if (image !== undefined) {
        vendorDetails.image = image
          ? String(image).trim()
          : null;
      }
...
const parseDashboardDateRange = (startDate, endDate) => {
  ...
  const startParts = parseIsoDateParts(
    String(startDate).trim()
  );

  const endParts = parseIsoDateParts(
    String(endDate).trim()
  );
  ...
  try {
    ...
    return res.status(200).json({
      ...
      recentLoyaltyPrograms:
        recentPrograms.map(mapLoyaltyProgramCard),
    });
  } catch (error) {
    console.error(
...
const QR_CODE_DIRECTORY = path.join(
  process.cwd(),
  'uploads',
  'loyalty',
  'qr-codes'
...
const QR_CODE_PUBLIC_DIRECTORY =
  '/uploads/loyalty/qr-codes';
...
const sanitizeFileName = (value) => {
  ...
  return sanitized || 'loyalty-program';
};
...
const getQrCodeFileName = (
  ...
  const programSlug =
    sanitizeFileName(programName);

  const tokenSuffix = String(
    qrCodeToken
  ).slice(0, 12);

  return `loyalty-program-${programSlug}-${tokenSuffix}.png`;
};

const ensureQrCodeDirectory = async () => {
...
const getQrCodeUrl = (fileName) => {
  return `${QR_CODE_PUBLIC_DIRECTORY}/${fileName}`;
};

const toLoyaltyProgramResponse = (
  program,
  qrCodeUrl = null
) => {
...
const validateNotificationSettings = (body) => {
  ...
  values.notificationStarField =
    notificationStarField
      ? String(notificationStarField).trim()
      : 'STAR_COUNT';
  ...
  values.notificationConditionOperator =
    String(
      notificationConditionOperator
    ).toUpperCase();
  ...
  values.notificationComparisonOperator =
    String(
      notificationComparisonOperator
    ).toUpperCase();
  ...
  values.notificationComparisonValue =
    comparisonValue;
  ...
  values.notificationAction =
    String(notificationAction).trim();

  values.notificationTemplate =
    String(notificationTemplate).trim();
...
exports.createLoyaltyProgram = async (
  req,
  res
) => {
  try {
    ...
    const notificationResult =
      validateNotificationSettings(req.body);
    ...
    /*
     * hasPin is the current flag.
     * enablePinVerification is retained as a
      ...
      const loyaltyProgram =
        await LoyaltyProgram.create({
          ...
          programRules: programRules
            ? String(programRules).trim()
            : null,
          notificationEnabled:
            n.notificationEnabled,
          notificationStarField:
            n.notificationStarField,
          notificationConditionOperator:
            n.notificationConditionOperator,
          notificationComparisonOperator:
            n.notificationComparisonOperator,
          notificationComparisonValue:
            n.notificationComparisonValue,
          notificationAction:
            n.notificationAction,
          notificationTemplate:
...
exports.getRecentLoyaltyPrograms =
  async (req, res) => {
    try {
      const vendorId = req.user.id;
...
exports.activateLoyaltyProgram =
  async (req, res) => {
    try {
      const programId =
      ...
      if (
        ...
        return res.status(400).json({
          message:
            'A valid loyalty program id is required.',
        });
      }
      ...
      if (program.isActive) {
        return res.status(400).json({
          message:
            'Loyalty program is already active.',
        });
      }
      ...
      return res.status(200).json({
        message:
          'Loyalty program activated successfully.',
        loyaltyProgram: program,
      });
    } catch (error) {
      console.error(
        'Activate loyalty program error:',
        error
      );
...
exports.deactivateLoyaltyProgram =
  async (req, res) => {
    try {
      const programId =
        Number(req.params.programId);
      ...
      if (!program.isActive) {
        return res.status(400).json({
          message:
            'Loyalty program is already inactive.',
        });
      }
      ...
      console.error(
        'Deactivate loyalty program error:',
        error
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!program.isActive) {
        return res.status(400).json({
          message:
            'Cannot generate a PIN for an inactive loyalty program.',
        });
      }
      ...
      if (
        ...
        return res.status(400).json({
          message:
            'PIN verification is not enabled for this loyalty program.',
        });
      }
      ...
      return res.status(201).json({
        success: true,
        message:
          'Loyalty verification PIN generated successfully.',
        loyaltyProgramId:
          program.id,
        pin,
        expiresAt,
