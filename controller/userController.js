...
const resolveCurrentStars = (
    ...
    return Number(
        enrollments[0].starsCollected || 0
    );
};


const buildProgramDetails = (
    program
) => {

    const existingDetails =
        program.details &&
        typeof program.details === "object"
            ? program.details
            : {};
...
const getEnrollmentForUser = async (
    userId,
    vendorId
) => {
...
exports.getStoreById = async (
    ...
    try {
        ...
        if (vendorResult.errorStatus) {
            return res.status(
                vendorResult.errorStatus
            ).json({
                message:
                    vendorResult.errorMessage
            });
        }
...
// =====================================================
//
// GET /api/user/stores/:vendorId/loyalty-programs/:programId
//
// =====================================================

exports.getLoyaltyProgramDetails = async (
    req,
    res
) => {
    try {
        ...
        if (
            ...
            return res.status(400).json({
                message:
                    "A valid store id and program id are required."
            });
        }
